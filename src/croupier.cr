...
module Croupier
  ...
  class Task
    ...
    def stale?
      ...
      position = TaskManager.tasks[@output].index(self) || 0
      ...
      @stale = (
        ...
        @inputs.any? { |input|
          TaskManager.tasks.has_key?(input) &&
            TaskManager.tasks[input].any?(&.stale?)
        } ||
        # It shares output with other tasks and an earlier one is stale
        (
          TaskManager.tasks[@output].size > 1 &&
          TaskManager.tasks[@output][..position - 1].any?(&.stale?)
        )
      )
    end
  ...
  class TaskManager
    ...
    @@tasks = {} of String => Array(Croupier::Task)
