...
module Croupier
  ...
  class Task
    ...
    def self.all_inputs
      ...
      @@tasks.values.flatten.each do |task|
        all += task.@inputs
      end
      all
    end
    ...
    def self._dependencies(outputs : Array(String))
      ...
      outputs.each do |output|
        if @@tasks.has_key? output
          result << output
          @@tasks[output].each do |task|
            result += _dependencies(task.@inputs).to_set
          end
        end
      end
    ...
    def self._run_tasks(outputs, run_all : Bool = false)
      outputs.each do |output|
        if @@tasks.has_key?(output) && (run_all || @@tasks[output].any?(&.@stale))
          @@tasks[output].each do |task|
            data = task.run
            # FIXME, only calculate the last one
            @@next_run[output] = Digest::SHA1.hexdigest(data)
          end
        end
      end
      save_run
    ...
    def self.run_tasks_parallel
      mark_stale_inputs
      loop do
        stale_tasks = @@tasks.values.select(&.any?(&.stale?)).to_set
        if stale_tasks.empty?
          break
        end
        batch = stale_tasks.select(&.all?(&.ready?)).flatten
    ...
    # * If it shares output with other tasks and an earlier one is stale
    ...
    def stale?
      ...
      position = @@tasks[@output].index(self) || 0
      ...
      @stale = (
        ...
        @inputs.any? { |input|
          @@tasks.has_key?(input) &&
            @@tasks[input].any?(&.stale?)
        } ||
        # It shares output with other tasks and an earlier one is stale
        (
          @@tasks[@output].size > 1 &&
          @@tasks[@output][..position - 1].any?(&.stale?)
        )
      )
    end
