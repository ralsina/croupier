...
module Croupier
  ...
  class Task
    ...
    #
    # Staleness is tri-state: nil=unknown (compute on-demand), true=stale,
    # false=fresh. TaskManager.propagate_staleness pre-computes this for
    # O(V+E) performance, and running a task sets it to false. This method
    ...
    def stale?
      # Staleness is known (propagated, or the task ran): report it.
      return @stale_atomic.get unless @stale.nil?
